...
/* FNMLNGTH = constant to define the max length of filenames */
...
int main(int argc, Char *argv[])
{
	...
	int p,i1, my_rank, tag = 0;
	int rowstot, rowsperp, rowcount = 0;
	long i, j;
	double v;

	/*----Added for timing analysis------*/
	time_t t0,t1;
	long makedist_loop;
	long init_prcd;
	long openfile_prcd;
...
#endif
	// t0 = time(NULL);
	init(argc, argv);    /* in phylip.c, used by all programs to initialize things. No changes need to be made here */
	// t1 = time(NULL);
	init_prcd = (long) t1 - t0;

//	printf("My rank = %d\n",my_rank);

	openfile(&infile,INFILE,"//home//cse456//sanks87//input file","r",argv[0],infilename);	
//	printf("Number of processors = %d\n",p);
	// t0 = time(NULL); 
	openfile(&outfile,OUTFILE,"//home//cse456//sanks87//output file","a+",argv[0],outfilename);
	t1 = time(NULL);
	//  openfile_prcd = (long) t1 - t0;

	ibmpc = IBMCRT;	/* constants within Phylip.h. Turns on screen controls settings based on the OS user is running */
	ansi = ANSICRT;	/* same as imbpc */
	mulsets = false;
	datasets = 1;
	firstset = true;
	...
	//  t1 = time(NULL);
	weight_open = (long) t1 - t0;

	t0= clock();	
	//  t0 = time(NULL);
	for (ith = 1; ith <= datasets; ith++) {			/* datasets is set by initdatasets function. Called within doinit(). doinit() called ln 1286 */
		ttratio = ttratio0;								/* ttratio may be set within the initratio function. called within getOptions() */
		getinput(); 									/* gets the menu inputs */
		if (ith == 1)
			firstset = false;
	...
	if(my_rank == 0)
	{
		printf("SHANKAR CHKPOINT 1\n");
		rowsperp = spp/(p-1);

		// here the matrix is split into equal rows and sent to the worker processes except for the one unequal portion
		...
		for(i1=1; i1<p; i1++)
		{
			...
			if(i1==1)
			{
				rowstot = rowsperp + (spp-(p-1)*rowsperp);
			}

			//MPI_Scatter(&rowcount, 1, MPI_INT, &rowstot, 1, MPI_INT, 0, MPI_COMM_WORLD);
			//MPI_Scatter(&rowstot, 1, MPI_INT, &result[rowcount], 1, MPI_INT, 0, MPI_COMM_WORLD);

			//int MPI_Send( void *buf, int count, MPI_Datatype datatype, int dest, 
			//            int tag, MPI_Comm comm )

			MPI_Send(&rowcount, 1, MPI_INT, i1, tag, MPI_COMM_WORLD);
			MPI_Send(&rowstot, 1, MPI_INT, i1, tag, MPI_COMM_WORLD);
			rowcount = rowcount + rowstot;
		}

		printf("SHANKAR CHKPOINT 2\n");
		//here the multiplied pieces are received back again by the source
		for(i1=1; i1<p; i1++)
		{
			MPI_Recv(&rowcount, 1, MPI_INT, i1, tag, MPI_COMM_WORLD, &status);
			MPI_Recv(&rowstot, 1, MPI_INT, i1, tag, MPI_COMM_WORLD, &status);

			printf("%d\t%d\n",rowcount,rowstot);
			MPI_Recv(&d[rowcount][0], rowstot*spp, MPI_DOUBLE, i1, tag, MPI_COMM_WORLD, &status);
		}
		printf("SHANKAR CHKPOINT 4\n");
...
*/
	...
	//	fclose(outfile);
			...
			//t1= clock();
		//  t1 = time(NULL);
		//makedist_loop = (float) (t1 - t0)/CLOCKS_PER_SEC;
#ifdef MAC
...
#endif
		 	//printf("hello");
		//  printf("Time in init_prcd:     %ld\n", (long) init_prcd    );
		//  printf("Time in openfile_prcd: %ld\n", (long) openfile_prcd);
		//  printf("Time in doinit_prcd:   %ld\n", (long) doinit_prcd  );
		//  printf("Time in weight_open:   %ld\n", (long) weight_open  ); 
		printf("Time in makedist_loop: %f\n", (float) makedist_loop);

	}
	else
	{

		//for the worker processes,receive the pieces,invoke the multiply function and send back the multiplied portion to master
		printf("SHANKAR CHKPOINT 3\n");

		MPI_Recv(&rowcount, 1, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
		MPI_Recv(&rowstot, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);

		//matmul(rowstot, rowcount);

		//worker part
		for (i = rowcount; i < rowstot+rowcount; i++) {
			if (progress) {
				printf("    ");
				for (j = 0; j < nmlngth; j++)
					putchar(nayme[i - 1][j]);
...
/*
		for (i = rowcount; i < rowstot+rowcount; i++) {
			for (j = i + 1; j <= spp; j++) {
				if(d[i][j]>0)
					printf("SHANKAR SOMETHING WRONG\n");
			}
		}
*/	
		//printf("My id is %d\n",my_rank);
		...
		MPI_Send(&rowcount,1, MPI_INT, 0, tag, MPI_COMM_WORLD);
		MPI_Send(&rowstot, 1, MPI_INT, 0, tag, MPI_COMM_WORLD);
		MPI_Send(&d[rowcount][0], rowstot*spp, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);
		//end_time =  MPI_Wtime();
	}
